...
def initial():
    ...
    libopt.add_argument(
        "--nit", type=int, default=100, help="I_max,# of maximum SCA loops"
    )
    libopt.add_argument(
        "--Jmax", type=int, default=50, help="# of maximum Gibbs Outer loops"
    )
    libopt.add_argument(
        "--threshold",
        type=float,
    ...
    libopt.add_argument(
        "--tau", type=float, default=1, help=r"\tau, the SCA regularization term"
    )
    ...
    libopt.add_argument(
        ...
        type=int,
        default=2,
        help=r"=1 if concentrated devices+ euqal dataset;\
    ...
    libopt.add_argument(
        "--local_bs", type=int, default=0, help="0 for no effect,Local Bath size B"
    )
...
if __name__ == "__main__":
    ...
    libopt.alpha_direct = 3.76  # PL = path loss component
    ...
    for i in range(libopt.trial):
        ...
        if libopt.set == 1:
            ...
            print(sum(libopt.K))
            libopt.dx2 = (
                np.random.rand(int(libopt.M - np.round(libopt.M / 2))) * libopt.range
                - libopt.range
            )  # [100,100+range]
        else:  # with data heterogeneity and channel heterogeneity
            ...
            libopt.lessuser = np.random.choice(
                libopt.M, size=lessuser_size, replace=False
            )
        ...
        libopt.dx1 = (
            np.random.rand(int(np.round(libopt.M / 2))) * libopt.range - libopt.range
        )  # [-range,0]
        libopt.dx = np.concatenate((libopt.dx1, libopt.dx2))
        libopt.dy = np.random.rand(libopt.M) * 20 - 10
        libopt.d_UR = (  # distance between user and RIS
            (libopt.dx - libopt.RIS[0]) ** 2
            + (libopt.dy - libopt.RIS[1]) ** 2
            + libopt.RIS[2] ** 2
        ) ** 0.5
        libopt.d_RB = np.linalg.norm(libopt.BS - libopt.RIS)   # distance between BS and RIS
        libopt.d_RIS = libopt.d_UR + libopt.d_RB
        libopt.d_direct = (
            (libopt.dx - libopt.BS[0]) ** 2
            + (libopt.dy - libopt.BS[1]) ** 2
            + libopt.BS[2] ** 2
        ) ** 0.5
        libopt.PL_direct = (  # The free-space path loss model is adopted for the device-PS direct channels
            BS_Gain
            * User_Gain
            * (3 * 10**8 / fc / 4 / np.pi / libopt.d_direct) ** libopt.alpha_direct
        )
        libopt.PL_RIS = (  # The path loss of the cascaded device-RIS-PS channel
            BS_Gain
            * User_Gain
            * RIS_Gain
            * libopt.L**2
            * d_RIS**2
            / 4
            / np.pi
            * (3 * 10**8 / fc / 4 / np.pi / libopt.d_UR) ** 2
            * (3 * 10**8 / fc / 4 / np.pi / libopt.d_RB) ** 2
        )
        # channels
        h_d = (
            np.random.randn(libopt.N, libopt.M)
            + 1j * np.random.randn(libopt.N, libopt.M)
        ) / 2**0.5
        h_d = h_d @ np.diag(libopt.PL_direct**0.5) / ref
        H_RB = (
            np.random.randn(libopt.N, libopt.L)
            + 1j * np.random.randn(libopt.N, libopt.L)
        ) / 2**0.5
        h_UR = (
            np.random.randn(libopt.L, libopt.M)
            + 1j * np.random.randn(libopt.L, libopt.M)
        ) / 2**0.5
        ...
        if Proposed:
            ...
            [x_store, obj_new, f_store, theta_store] = Gibbs(
                libopt, h_d, G, x, True, True
            )
        ...
        #     obj_DC_RIS, F_DC_RIS, theta_DC_RIS = DC_RIS.DC_RIS(
        #         libopt, h_d, G, libopt.verbose
        #     )
        ...
        #     obj_new_NORIS, x_store_NORIS, f_store_NORIS = DC_DS.DC_NORIS(
        #         libopt, h_d, gamma, libopt.verbose
        #     )
        #     end = time.time()
        #     print("Running time: {} seconds".format(end - start))
        ...
        # else:
        #     obj_new_NORIS = [0]
        #     x_store_NORIS = np.zeros(
        #         [
        #             libopt.M,
        #         ]
        #     )
        #     f_store_NORIS = np.zeros(
        #         [
        #             libopt.N,
        #         ]
        #     )
        ...
        # if Proposed:
        #     print(
        #         "Algorithm2:{:.6f} Algorithm1:{:.6f} NoRIS:{:.6f} NoDS:{:.6f} DG:{:.6f}".format(
        #             obj_new[libopt.Jmax],
        #             obj_new[0],
        #             obj_new_NORIS[0],
        #             obj_DC_RIS[0],
        ...
        result, _ = flow.learning_flow(
            libopt, Noiseless, Proposed, NoRIS, NoDS, SVD, h_d, G, dic
        )
