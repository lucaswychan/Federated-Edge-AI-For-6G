...
from air_comp import transmission
import train_script
...
def FedAvg_grad(w_glob, grad, device):  # similar to set_client_from_params
    ind = 0
...
def Learning_iter(
    ...
    net_glob.eval()
    acc_test, loss_test = train_script.test_model(
        net_glob, libopt, test_images, test_labels
    )
    ...
    for iter in range(libopt.epochs):
        ...
        for idx in idxs_users:
            ...
            w, loss, gradient = train_script.local_update(
                libopt,
                d,
                copy.deepcopy(net_glob).to(libopt.device),
                train_images,
                train_labels,
                idx,
                size,
        ...
        if trans_mode == 0:
            grad = np.average(
                copy.deepcopy(grad_store_per_iter),
                axis=0,
                weights=libopt.K[idxs_users] / sum(libopt.K[idxs_users]),
            )
        elif trans_mode == 1:
            grad = transmission(libopt, d, copy.deepcopy(grad_store_per_iter), x, f, h)
        #            print(mse)
        # if libopt.verbose:
...
def learning_flow(libopt, Noiseless, Proposed, NoRIS, NoDS, SVD, h_d, G, dic):
    x_store = dic["x_store"]
    f_store = dic["f_store"]
    ...
    train_images, train_labels, test_images, test_labels = train_script.Load_FMNIST_IID(
        libopt.M, libopt.K
    )
    net_glob = CNNMnist(num_classes=10, num_channels=1, batch_norm=True).to(
        libopt.device
    )
    ...
    if Noiseless:
        ...
        loss_train, accuracy_test, loss_test = Learning_iter(
            libopt,
            d,
            net_glob,
            w_glob,
        ...
        print(
            "result {}".format(
                result["accuracy_test"][len(result["accuracy_test"]) - 1]
            )
        )
    ...
    if Proposed:
        ...
        print("selected clients = {}".format(idxs_users))
        loss_train1, accuracy_test1, loss_test1 = Learning_iter(
            libopt,
            d,
            net_glob,
            w_glob,
        ...
        print(
            "result {}".format(
                result["accuracy_test1"][len(result["accuracy_test1"]) - 1]
            )
        )
    ...
    if NoDS:
        ...
        net_glob.load_state_dict(w_glob)
        idxs_users = np.asarray(range(libopt.M))
        loss_train2, accuracy_test2, loss_test2 = Learning_iter(
            libopt,
            d,
            net_glob,
            w_glob,
        ...
        print(
            "result {}".format(
                result["accuracy_test2"][len(result["accuracy_test2"]) - 1]
            )
        )
    if NoRIS:
        ...
        loss_train3, accuracy_test3, loss_test3 = Learning_iter(
            libopt,
            d,
            net_glob,
            w_glob,
        ...
        print(
            "result {}".format(
                result["accuracy_test3"][len(result["accuracy_test3"]) - 1]
            )
        )
    ...
    if SVD:
        ...
        net_glob.load_state_dict(w_glob)
        idxs_users = np.asarray(range(libopt.M))
        loss_train5, accuracy_test5, loss_test5 = Learning_iter(
            libopt,
            d,
            net_glob,
            w_glob,
        ...
        print(
            "result {}".format(
                result["accuracy_test5"][len(result["accuracy_test5"]) - 1]
            )
        )
