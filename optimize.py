...
# Algorithm 1
def sca_fmincon(libopt, h_d, G, f, theta, x, K2, RISON):   # (25)
    N = libopt.N
    L = libopt.L
    I = sum(x)
    tau = libopt.tau
...
def find_obj_inner(libopt, x, K, K2, Ksum2, h_d, G, f0, theta0, RISON):
    ...
    if sum(x) == 0:
        ...
        f, theta, _ = sca_fmincon(
            libopt, h_d[:, index], G[:, :, index], f0, theta0, x, K2[index], RISON
        )
...
class Gibbs:
    def __init__(self, n_clients, n_receive_antennas, n_RIS_ele, Jmax, K, RISON):
    ...
    def optimize(self, h_d, G, x0, RISON):
        ...
        [obj_new[ind], x_store[ind, :], f, theta] = self.find_obj_inner(
            x, K, K2, Ksum2, h_d, G, None, None, RISON
        )
    ...
    def find_obj_inner(self, x, K, K2, Ksum2, h_d, G, f0, theta0, RISON, sigma):
        N = self.n_receive_antennas
        L = self.n_RIS_ele
        M = self.n_clients
        ...
        if sum(x) == 0:
            ...
            f, theta, _ = self.sca_fmincon(
                h_d[:, index], G[:, :, index], f0, theta0, x, K2[index], RISON
            )
    ...
    # Algorithm 1
    def sca_fmincon(self, h_d, G, f, theta, x, K2, RISON, tau, nit, threshold):   # (25)
        N = self.n_receive_antennas
        L = self.n_RIS_ele
        I = sum(x)
...
# Algorithm 2
def Gibbs(libopt, h_d, G, x0, RISON):
    # initial
    ...
    # the first loop
    ind = 0
    [obj_new[ind], x_store[ind, :], f, theta] = find_obj_inner(
        libopt, x, K, K2, Ksum2, h_d, G, None, None, RISON
    )
    ...
    for j in range(Jmax):
        ...
        for m in range(M):
            ...
            x_sam[m] = copy.deepcopy((x_sam[m] + 1) % 2)
            X_sample[m + 1, :] = copy.deepcopy(x_sam)
            Temp[m + 1], _, f_loop[m + 1], theta_loop[m + 1] = find_obj_inner(
                libopt,
                x_sam,
                K,
                K2,
