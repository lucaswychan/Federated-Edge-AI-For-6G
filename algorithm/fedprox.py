...
class FedProx(Algorithm):
    def __init__(self, act_prob, lr, lr_decay_per_round, batch_size, epoch, weight_decay, model_func, init_model, data_obj, n_param, max_norm, air_comp, noiseless, save_period, print_per, mu):
        super().__init__("FedProx", act_prob, lr, lr_decay_per_round, batch_size, epoch, weight_decay, model_func, init_model, data_obj, n_param, max_norm, air_comp, noiseless, save_period, print_per)
    ...
    def local_train(self, client: Client, inputs: dict):
        ...
        client.model = self._train_model(client.model, trn_x, trn_y, inputs["curr_round"], inputs["avg_model_param_tensor"])
    ...
    def aggregate(self, inputs: dict):
        ...
        if not self.noiseless:
            ...
            avg_mdl_param = np.sum(all_clients_param_list[selected_clnts_idx]*weight_list[selected_clnts_idx]/np.sum(weight_list[selected_clnts_idx]), axis=0)
        ...
        inputs["all_model"] = set_model(self.model_func(), np.sum(all_clients_param_list*weight_list/np.sum(weight_list), axis = 0), device)